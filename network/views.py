...
def list_posts(request, key):
    try:
        if key == "all":
            ...
            posts = Post.objects.filter(user__in=request.user.profile.get(
                user=request.user).following.all()).order_by("-timestamp").all()
            return JsonResponse([post.serialize() for post in posts], safe=False)
        else:
...
def profile(request, username):
    if request.method == "GET":
        ...
        follower = User.objects.get(
            username=json.loads(request.body).get("follower"))
